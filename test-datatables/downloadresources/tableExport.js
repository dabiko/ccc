...
(function ($) {
  $.fn.tableExport = function (options) {
    ...
    var defaults = {
      ...
      fileName:            chartinfo.title+' '+chartinfo.dater,
      htmlContent:         false,
      ignoreColumn:        [],
      ignoreRow:           [],
...
//                   html2canvas($('#myChart'), {
//            onrendered: function (canvas) {
//                var img = canvas.toDataURL("images/jpeg", 1.0);
...
//    });
 ...
 //hidden canvas            
  ...
  var reportPageHeight = $('#myChartprint').innerHeight();
  var reportPageWidth = $('#myChartprint').innerWidth();
  var width=800;
  var scaleFactor = width /  reportPageWidth;
  var height = reportPageHeight  * scaleFactor;
  ...
  var buffer = 100;
    ...
    var canvasWidth = $('canvas').innerWidth();
                  ...
                  var canvas = document.querySelector('#myChartprint');
                  canvas.toDataURL('images/png',1.0);
    pdfctx.drawImage(canvas, pdfctxX, pdfctxY, width, height);
            
            teOptions.doc = new jsPDF('p'
                                    ,
                                    defaults.jspdf.unit,
                                    'a4');
       ...
       ctx.drawImage(img,0,0,90,84);
            ...
            teOptions.doc.addImage(elem, 'PNG',5,5);   
            teOptions.doc.setFontSize(18);
            teOptions.doc.setFontStyle('bold');
            teOptions.doc.text(250, 60, 'CCC PLC');
            teOptions.doc.setFontSize(16);
            teOptions.doc.text(215, 80, chartinfo.titlemaker);
//            var date1= $(".start_date").val();
//            var  date2= $(".end_date").val();
            teOptions.doc.setFontSize(12);
            teOptions.doc.setFontStyle('normal');
            teOptions.doc.text(180, 100,report_header.EXTRA_TITLE);
            teOptions.doc.addImage($(pdfCanvas)[0], 'PNG', 0, 105);
           // teOptions.doc.addPage();
        ...
        loadImages(teOptions, function () {
          $(el).filter(function () {
            ...
            atOptions.margin = {left: 20, right: 10, top: 430, bottom: 10};
