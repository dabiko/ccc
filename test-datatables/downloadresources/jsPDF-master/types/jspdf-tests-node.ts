...
function test_images() {
  ...
  const createPDF = function(imgData: string) {
    const doc = new jsPDF();
    doc.addImage(imgData, "JPEG", 10, 10, 50, 50, "monkey"); // Cache the images using the alias 'monkey'
    doc.addImage("monkey", 70, 10, 100, 120); // use the cached 'monkey' images, JPEG is optional regardless
    doc.addImage({
      imageData: imgData,
      rotation: -20,
      x: 10,
